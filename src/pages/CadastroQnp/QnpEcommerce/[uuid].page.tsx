...
const UPDATE_ONE_QNP = gql`
  mutation updateOneQnp($content: JSON!, $clientId: Int!) {
...
const CLIENT = gql`
  mutation regitserClient($uuid: String!, $content: JSON!) {
    upsertOneClient(
      where: { userUuid: $uuid }
...
export default function QnpEcommerce({ params }: any) {
  const router = useRouter()
  const token = router.query.uuid as string
  ...
  const [form, setForm] = useState([
...

    {
      ...
      required: "Campo obrigatório",
      placeholder: "Descreva aqui...",
      error: <>{errors.gatewayPayment && <span style={{ color: "red" }}>{errors.gatewayPayment.message}</span>}</>,
    },
    {
      fieldType: "text",
      ...
      required: "Campo obrigatório",
      placeholder: "Descreva aqui...",
      error: <>{errors.marketplace && <span style={{ color: "red" }}>{errors.marketplace.message}</span>}</>,
    },
    {
      fieldType: "BigText",
      ...
      placeholder: "Descreva aqui...",
      error: (
        <>{errors.shippingConditions && <span style={{ color: "red" }}>{errors.shippingConditions.message}</span>}</>
      ),
    },
    {
      fieldType: "radio",
      label: "Como você classifica a sua logística hoje",
      name: "selectScoreLogistic",
      required: "Campo obrigatório",
      placeholder: "Selecione",
      options: [
        { value: "01", label: "01" },
    ...
    },
    {
      ...
      name: "accountableSuport",
      required: "Campo obrigatório",
      placeholder: "Descreva aqui...",
      error: (
        <>{errors.accountableSupport && <span style={{ color: "red" }}>{errors.accountableSupport.message}</span>}</>
      ),
    },
    {
      fieldType: "BigText",
      ...
      required: "Campo obrigatório",
      placeholder: "Descreva aqui...",
      error: <>{errors.crmDescription && <span style={{ color: "red" }}>{errors.crmDescription.message}</span>}</>,
    },
    {
      fieldType: "text",
      ...
      error: <>{errors.historyPurchase && <span style={{ color: "red" }}>{errors.historyPurchase.message}</span>}</>,
    },
  ...
  return (
    ...
    <Container id='cadastro-qnp' name='cadastro-qnp'>
      <input type='hidden' defaultValue={"E-Commerce"} {...register("qnpType" as keyof Register)} />
      ...
      <div ref={sliderRef} className='keen-slider'>
        ...
        >
          ...
          <Content
            style={{
              minHeight: windowHeight < 800 ? "250rem" : "180.375rem",
              display: "flex",
              flexDirection: "column",
            ...
            onClick={handleSubmit(onSubmit)}
          >
            {content.map((field, index) => index >= 12 && index < 25 && field)}
            <CustomProgress
              width='100%'
              progress={porcentagemConclusao}
              height='0.625rem'
        ...
        >
          ...
          <Content
            ...
            onClick={handleSubmit(onSubmit)}
          >
            {content.map((field, index) => index >= 25 && index < 32 && field)}
            <CustomProgress
              width='100%'
              progress={porcentagemConclusao}
              height='0.625rem'
        ...
        >
          ...
          >
            {content.map((field, index) => index >= 32 && index < 40 && field)}
            <CustomProgress
              width='100%'
              progress={porcentagemConclusao}
              height='0.625rem'
        ...
        >
          ...
          >
            {content.map((field, index) => index >= 40 && index < 49 && field)}
            <CustomProgress
              width='100%'
              progress={porcentagemConclusao}
              height='0.625rem'
