...
export function PrimitivesCep({
  ...
  const masks = ["99999-999"]; // Máscaras que serão usadas

  const [m, setM] = useState("99999-999");
  const handleInput = (current: string) => { // Função para lidar com mudanças no input
    const maskLength = current.replace(/\D/g, "").length;
    const isValid = maskLength <= 8;
    const m = masks[isValid ? 0 : 1];
  ...
  const handleBlur = () => { // Função para lidar com o evento onBlur
    if (onBlur) {
