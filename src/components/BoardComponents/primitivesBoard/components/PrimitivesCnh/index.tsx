...
export function PrimitivesCnh({
  ...
  const [m, setM] = useState("00000000000")

  const inputFn = (current: string) => {
    const masks = ["00000000000"]
    // Se current for vazio, limpa o valor e retorna cedo
    if (current === "") {
      setValue("")
      setM(masks[0])
    ...
    const isValid = maskLength <= 11 // Verifica se a entrada é válida

    if (isValid) {
      const m = masks[0] // Sempre usa a primeira máscara
      const maskedValue = mask(current, m)
      setValue(maskedValue)
      setM(m)
      if (onChange) onChange(maskedValue)
      if (onBlur) onBlur(maskedValue)
    }
  }
