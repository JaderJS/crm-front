...
import { Add, AddCircle, ArrowRight, CloseCircle, Edit, TextBlock, TickSquare, Trash } from "iconsax-react"
...
const FIND_UNIQUE_PRIMITIVE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPCHECKBOX = gql`
  query findUniquePrimitiveFieldInCreateOneFieldInPopUpCHECKBOX($type: FieldType!) {
...
const UPSERT_ONE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPCHECKBOX = gql`
  mutation upsertOneFieldInCreateOneFieldInPopUpCHECKBOX(
    $id: Int!
...
export function PopUpCheckbox({ columnId, typeField, field, uuid, refetch }: IPopUpShortTextProps) {
  ...
  const [primitiveField] = useLazyQuery<PrimitiveField>(
    FIND_UNIQUE_PRIMITIVE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPCHECKBOX,
  )
  const [upsertOneField] = useMutation(UPSERT_ONE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPCHECKBOX)
          ...
          <Content onSubmit={handleSubmit(submit)}>
            ...
            >
              ...
              {fieldsOptions.map((option, index) => (
                <div
                  key={index}
                  style={{ display: "flex", flexDirection: "row", gap: "1rem", width: "100%", alignItems: "center" }}
                >
                  <PrimitivesBoard
                    key={index}
                    typeField='SHORTTEXT'
