...
import { ArrowRight, Clipboard, CloseCircle, DirectInbox, Edit, Link, People, TextBlock } from "iconsax-react"
...
import { PrimitiveFields, Field } from "@/types"
import { z } from "zod"
...
const FIND_UNIQUE_PRIMITIVE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUP_INVEST = gql`
  query findUniquePrimitiveFieldInCreateOneFieldInPopUpInvest($type: FieldType!) {
...
const UPSERT_ONE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUP_INVEST = gql`
  mutation upsertOneFieldInCreateOneFieldInPopUpInvest(
    $id: Int!
...
const fieldType = {
    createLabel: <><People /> Investidor</>,
    editLabel: <Edit />,
    type: "INVEST"
}

export function PopUpInvest({ columnId, field, refetch }: PoPupInvestProps) {
    ...
    const [primitiveField] = useLazyQuery<PrimitiveField>(FIND_UNIQUE_PRIMITIVE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUP_INVEST)
    const [upsertOneField] = useMutation(UPSERT_ONE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUP_INVEST)
