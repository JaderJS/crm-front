...
import { ArrowRight, CloseCircle, Edit, Location, Sms, TextBlock } from "iconsax-react"
...
const FIND_UNIQUE_PRIMITIVE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPEMAIL = gql`
  query findUniquePrimitiveFieldInCreateOneFieldInPopUpEMAIL($type: FieldType!) {
...
const UPSERT_ONE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPEMAIL = gql`
  mutation upsertOneFieldInCreateOneFieldInPopUpEMAIL(
    $id: Int!
...
export function PopUpEmail({ columnId, typeField, field, uuid, refetch }: IPopUpShortTextProps) {
  ...
  const [primitiveField] = useLazyQuery<PrimitiveField>(FIND_UNIQUE_PRIMITIVE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPEMAIL)
  const [upsertOneField] = useMutation(UPSERT_ONE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPEMAIL)
