...
const FIND_UNIQUE_PRIMITIVE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPNOMEDOPOPUP = gql`
  query findUniquePrimitiveFieldInCreateOneFieldInPopUpNomeDoPopUp($type: FieldType!) {
...
const UPSERT_ONE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPNOMEDOPOPUP = gql`
  mutation upsertOneFieldInCreateOneFieldInPopUpNomeDoPopUp(
    $id: Int!
...
export function PopUpShortText({ columnId, typeField, field, uuid, refetch }: IPopUpShortTextProps) {
  ...
  const [primitiveField] = useLazyQuery<PrimitiveField>(
    FIND_UNIQUE_PRIMITIVE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPNOMEDOPOPUP,
  )
  const [upsertOneField] = useMutation(UPSERT_ONE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPNOMEDOPOPUP)

  const submit = async (data: Data) => {
    const type = "NOMEPRIMTIVODOBACKEND"
