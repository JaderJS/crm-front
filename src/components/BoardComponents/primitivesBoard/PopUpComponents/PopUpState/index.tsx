...
import { ArrowRight, CloseCircle, Edit, Location, TextBlock } from "iconsax-react"
...
const FIND_UNIQUE_PRIMITIVE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPSTATE = gql`
  query findUniquePrimitiveFieldInCreateOneFieldInPopUpSTATE($type: FieldType!) {
...
const UPSERT_ONE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPSTATE = gql`
  mutation upsertOneFieldInCreateOneFieldInPopUpSTATE(
    $id: Int!
...
export function PopUpState({ columnId, typeField, field, uuid, refetch }: IPopUpShortTextProps) {
  ...
  const {
    ...
    defaultValues: {
      title: "Novo campo de estado",
      description: "Descrição do campo de estado",
      require: false,
    },
  })
  ...
  const [primitiveField] = useLazyQuery<PrimitiveField>(
    FIND_UNIQUE_PRIMITIVE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPSTATE,
  )
  const [upsertOneField] = useMutation(UPSERT_ONE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPSTATE)
