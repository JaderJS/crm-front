...
import { ArrowRight, Clipboard, CloseCircle, Edit, Image as IconImage, TextBlock } from "iconsax-react"
...
const FIND_UNIQUE_PRIMITIVE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUP_IMAGE = gql`
  query findUniquePrimitiveFieldInCreateOneFieldInPopUpImage($type: FieldType!) {
...
const UPSERT_ONE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUP_IMAGE = gql`
  mutation upsertOneFieldInCreateOneFieldInPopUpImage(
    $id: Int!
...
export function PopUpImage({ columnId, field, refetch }: PoPupImageProps) {
  ...
  const [primitiveField] = useLazyQuery<PrimitiveField>(FIND_UNIQUE_PRIMITIVE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUP_IMAGE)
  const [upsertOneField] = useMutation(UPSERT_ONE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUP_IMAGE)
      ...
      <Dialog.Portal>
        ...
        <PopUpChildren>
          ...
          <Content onSubmit={handleSubmit(submit)}>
            <FormText label='Titulo*' register={register("title")} error={errors.title} />
            <FormText label='Descrição do campo' register={register("description")} error={errors.description} />
            <FormSwitch label='Preenchimento obrigatório?' name='require' control={control} error={errors.require} />
