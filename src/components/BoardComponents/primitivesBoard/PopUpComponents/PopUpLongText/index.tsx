...
import { ArrowRight, CloseCircle, Edit, TextBlock, TextalignJustifycenter } from "iconsax-react"
...
const FIND_UNIQUE_PRIMITIVE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPLONGTEXT = gql`
  query findUniquePrimitiveFieldInCreateOneFieldInPopUpLongText($type: FieldType!) {
...
const UPSERT_ONE_FIELD_IN_POPUPLONGTEXT = gql`
mutation upsertOneFieldInPopUpLongText($id:Int!, $create:FieldUncheckedCreateInput!, $update: FieldUncheckedUpdateInput!){
...
export function PopUpLongText({ columnId, typeField, field, uuid, refetch }: IPopUpShortTextProps) {
  ...
  const {
    ...
    defaultValues: {
      title: "Novo campo de texto longo",
      description: "Descrição informativa sobre o texto longo",
      require: false,
    },
  })
  ...
  const [upsertOneField] = useMutation(UPSERT_ONE_FIELD_IN_POPUPLONGTEXT)
  const [primitiveField] = useLazyQuery<PrimitiveField>(
    FIND_UNIQUE_PRIMITIVE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPLONGTEXT,
  )
