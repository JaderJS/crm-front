...
const FIND_UNIQUE_PRIMITIVE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPCITY = gql`
  query findUniquePrimitiveFieldInCreateOneFieldInPopUpCity($type: FieldType!) {
...
const UPSERT_ONE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPCITY = gql`
  mutation upsertOneFieldInCreateOneFieldInPopUpCity(
    $id: Int!
...
export function PopUpCity({ columnId, typeField, field, uuid, refetch }: IPopUpShortTextProps) {
  ...
  const {
    ...
    defaultValues: {
      title: "Novo campo de cidade",
      description: "Descrição do campo de cidade",
      require: false,
    },
  })
  ...
  const [primitiveField] = useLazyQuery<PrimitiveField>(
    FIND_UNIQUE_PRIMITIVE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPCITY,
  )
  const [upsertOneField] = useMutation(UPSERT_ONE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUPCITY)
