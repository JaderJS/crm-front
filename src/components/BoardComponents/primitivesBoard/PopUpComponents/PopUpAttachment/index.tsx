...
import { ArrowRight, Clipboard, CloseCircle, Edit, TextBlock } from "iconsax-react"
...
const FIND_UNIQUE_PRIMITIVE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUP_ATTACHMENT = gql`
  query findUniquePrimitiveFieldInCreateOneFieldInPopUpAttachment($type: FieldType!) {
...
const UPSERT_ONE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUP_ATTACHMENT = gql`
  mutation upsertOneFieldInCreateOneFieldInPopUpAttachment(
    $id: Int!
...
const fieldType = {
    createLabel: <><Clipboard /> Anexo</>,
    editLabel: <Edit />,
    type: "ATTACHMENT"
}

export function PopUpAttachment({ columnId, field, refetch }: PoPupAppointmentProps) {
    ...
    const [primitiveField] = useLazyQuery<PrimitiveField>(FIND_UNIQUE_PRIMITIVE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUP_ATTACHMENT)
    const [upsertOneField] = useMutation(UPSERT_ONE_FIELD_IN_CREATE_ONE_FIELD_IN_POPUP_ATTACHMENT)
